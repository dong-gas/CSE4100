...
void Hash_Destructor(struct hash_elem *e, void *aux) {  // clear destructor
    // 메모리 free
    free(e);
    return;
}
...
void Hash_Insert(struct hash *Hash) {
    // val 삽입
    int val;
    scanf("%d", &val);

    struct hash_elem *u = malloc(sizeof(struct hash_elem));
    u->data = val;
...
void Hash_Replace(struct hash *Hash) {
    // 있으면 대체, 없으면 삽입
    int val;
    scanf("%d", &val);
...
void Hash_Find(struct hash *Hash) {
    // val 있으면 찾아서 출력
    int val;
    scanf("%d", &val);
    ...
    struct hash_elem *elem = hash_find(Hash, u);
    if (elem) printf("%d\n", elem->data);
    return;
}
...
/* Iteration. */
void act_square(struct hash_elem *e, void *aux) {
    // 제곱
    e->data = e->data * e->data;
    return;
}

void act_triple(struct hash_elem *e, void *aux) {
    // 세제곱
    e->data = e->data * e->data * e->data;
    return;
}

void Hash_Apply(struct hash *Hash) {
    // 제곱 or 세제곱 입력받은 대로 적용하기
    char action[10];
    scanf("%s", action);
    if (!strcmp(action, "square")) hash_apply(Hash, act_square);
    else if (!strcmp(action, "triple")) hash_apply(Hash, act_triple);
    return;
}

/* Information. */
void Hash_Size(struct hash *Hash) {
    // size 출력
    printf("%zu\n", hash_size(Hash));
    return;
}

void Hash_Empty(struct hash *Hash) {
    // empty 여부
